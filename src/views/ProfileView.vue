  ...
  <div id="profile-view" class="section">
    <div class="box has-text-centered">
      ...
      <div class="buttons is-centered has-addons">
        <button class="button is-danger is-selected is-italic" @click="beJobSeeker()">I wanna find a job</button>
        <button class="button" disabled>Or</button>
        <button class="button is-danger is-selected is-italic" @click="beCompany()">I wanna recruit talent</button>
      </div>
    </div>
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    console.log(response.data.is_company)
        if (response.data.is_company == "Not Created") {
            // push router to create company or job seeker profile
